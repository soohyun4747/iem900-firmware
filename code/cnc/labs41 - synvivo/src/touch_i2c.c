...
#define Touch_mark(x, y)	( Touch_x_data>(x) ) && ( Touch_x_data<(x)+159 ) && ( Touch_y_data>(y) ) && ( Touch_y_data<(y)+57 )	
...
{
	if	( Touch_mark(600,400) )			{ mode_value = device_set;	buzzer_set = 1;	}
...
u8 Rx_Buffer[1];
  ...
  if ( Rx_Buffer[0] == 85)  touch_version = 0;  //기존
  else                        touch_version = 1;  //신형
}
